/****************************************************************************
 * arch/arm/src/imx6/imx8_serial.c
 *
...
static inline uint32_t imx8_serialin(struct imx8_uart_s *priv,
              uint32_t offset);
static inline void imx8_serialout(struct imx8_uart_s *priv, uint32_t offset,
              uint32_t value);
...
static inline uint32_t imx8_serialin(struct imx8_uart_s *priv, uint32_t offset)
{
...
static inline void imx8_restoreuartint(struct imx8_uart_s *priv, uint32_t ucr1)
{
...
static int imx8_setup(struct uart_dev_s *dev)
{
  ...
  ret = imx8_uart_configure(priv->uartbase, &config);
  ...
  if (ret == OK)
    {
      ...
      /* arm64_gic_irq_trigger(priv->irq, 0); */
  ...
  for (tmp = 1000 ; tmp > 0 ; tmp--)
    {
      ...
      if ((getreg32(IMX8_CONSOLE_VBASE + UART_UTS_OFFSET) & UART_UTS_TXFULL) == 0)
        {
          break;
        }
    }
...
#ifdef IMX8_CONSOLE_VBASE
  imx8_waittxready();
  putreg32((uint16_t)ch, IMX8_CONSOLE_VBASE + UART_TXD_OFFSET);
#endif
}
